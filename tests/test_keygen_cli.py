...
def test_keygen_cli_passes_key_type(monkeypatch, tmp_path):
    ...
    def fake_store_private_key(path, key_type, overwrite=False):
        calls.append((path, key_type, overwrite))
    ...
    assert calls == [(str(key_path), "rsa", False)]
...
def test_keygen_cli_accepts_underscore_key_type(monkeypatch, tmp_path):
    ...
    assert calls == [(str(key_path), "secp256k", False)]


def test_keygen_cli_passes_overwrite(monkeypatch, tmp_path):
    calls = []
    ...
    monkeypatch.setattr(keygen, "store_private_key", fake_store_private_key)
    monkeypatch.setattr("sys.argv", ["keygen", "--path", str(key_path), "--overwrite"])

    keygen.main()

    assert calls == [(str(key_path), "ed25519", True)]
